...
| **Scripts** (`scripts/`) | Deterministic feature lifecycle (init/context/next-task/verify/ready/reconcile), compact all-active verification, evidence and loop-budget helpers, approvals/maintenance visibility, bounded pretriage and per-item opt-in, campaign accounting, credential preflight, health checks, sanitizer scanning, cross-model wrappers, optional agent capsules, worktree helper, backlog indexer, and test harness. |
