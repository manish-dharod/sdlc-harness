...
- Moves tasks `Backlog → Open`, and — during state hygiene — performs the `Review → Done` flip only after the all-tier receipt/evidence gates pass and the worktree is clean.
...
**Hand-off / state transitions it owns:** the normal transition for a code-bearing task is `Claimed → Review`, **not** `Claimed → Done`. Builder writes the diff, runs verification, and hands off to `/feature-review`. A task may only reach `Done` after every routed reviewer mode + `security` have cleared the exact committed diff and all P0/P1 findings are resolved; the `Review → Done` flip can be done by `builder` (after re-verification) or by `planner (Phase: plan)`. A current `Type: docs` task may transition straight to `Done` only when committed claim history proves a non-empty, fully owned documentation-only diff. Labels and routing-skip prose are not proof.
...
2. **Pre-review self-audit gate** — before handoff, create one task-scoped `Review evidence bundle` H2, name ≥3 plausible ways the diff could still be wrong, run one concrete check each, and end with an **AC-clause coverage table** (one row per clause of every AC, each pointing at diff evidence or marked `UNMET — routed to <FND/TASK/AMENDMENT>`). Keep the QA ledger, required application proof, and newest receipt citation in that same H2. A red required gate may not be prose-waived ("identical on the base" is a diagnosis, not a waiver) — fix it or file a FINDINGS/APPROVALS blocker.
3. **Adversarial-review iron law** — every current non-doc task on every tier needs a tracked allocator-nonce schema-v2 scoped `clear` receipt from the opposite tool before it can be treated as review-cleared or reach `Done`. Same-tool review, local transcripts, routing skips, and stale receipt citations do not satisfy the gate; any later task-owned product change requires a new receipt.
...
5. **Review hand-off rule** — only `builder` or `planner (Phase: plan)` performs the `Review → Done` flip, and only after `feature-reconcile` validates the current receipt/evidence bundle and strict hygiene is clean.
...
**Cross-model requirement (adversarial mode):** for every current non-doc task on every tier, same-tool adversarial review is **not** acceptable — Claude-authored work uses `scripts/adversary-review`, while Codex-authored work uses `scripts/claude-adversary-review`. The wrapper reviews a committed candidate and writes a tracked allocator-nonce schema-v2 scoped receipt only after a terminal `clear` verdict. The newest task-scoped evidence H2 must cite that receipt and contain the implementer/reviewer tool and model fields, the pre-review self-audit, zero-gap QA ledger, and any required application-verification proof. `scripts/feature-reconcile` validates the receipt, committed model pins, exact task identity, candidate ancestry, and absence of later task-owned product changes. If the required reviewer is unavailable, the task stays in `Review` and an `APPROVALS` entry opens with stop reason `NEEDS_CROSS_MODEL_REVIEWER` — **no silent fallback**, exemption file, or fabricated review.
...
- `/feature-review` spawns the **independent reviews in parallel** — `reviewer` in `quality`, `qa`, and `adversarial` modes plus `security` once — all looking at the same diff for different failure classes and writing findings back into the same feature folder. Risk routing skips irrelevant modes for a committed, classifier-verified docs-only task.
- A current non-doc task reaches `Done` only after every routed reviewer mode + `security` clears it, all P0/P1 findings are resolved, `feature-reconcile` validates the newest same-H2 review bundle and tracked opposite-tool receipt, and strict hygiene is clean.
