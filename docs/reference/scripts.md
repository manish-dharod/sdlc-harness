...
| `continuous-self-improvement-loop-verify` | `scripts/continuous-self-improvement-loop-verify [fast\|unit\|full]` | Portable public-repository packaging profile for the CSI mechanism: checks and sanitizes the CSI scripts/docs shipped here; `unit` and `full` also syntax-check the shipped shell entry points. Adopter-owned feature docs, insight ledgers, corpora, and deeper domain tests are intentionally not assumed. | `0` passed · non-zero on failure · `3` usage |
