...
| `campaign-ledger` | `scripts/campaign-ledger <start\|record\|status> ...` | Creates and updates a bounded local campaign ledger so unattended loops have explicit iteration, token, elapsed-time, and stop-reason accounting. | `0` updated/reported · `3` usage / invalid ledger |
