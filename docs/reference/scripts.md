...
| `feature-verify` | `scripts/feature-verify <slug> [fast\|unit\|full]`  ·  `scripts/feature-verify --all-active [fast\|unit\|full]` | Runs feature-level verification: framework checks, tier-aware file presence, declared credential preflight (non-fast modes), then auto-discovers and runs `scripts/<slug>-verify` if present. `--all-active` sweeps every active feature, emits a compact summary by default, and prints a feature's full log only on failure (`SDLC_ALL_ACTIVE_VERBOSE=1` restores verbose output). | `0` passed · `1` failed (or, with `--all-active`, any feature failed) · `2` usage / feature not found |
| `evidence-scaffold` | `scripts/evidence-scaffold <slug> <task-id> [fast\|unit\|full]` | Writes a bounded, task-scoped evidence skeleton with command, result, reviewer, and artifact placeholders. It never marks a check passing. | `0` scaffold written · `3` usage / feature missing |
| `loop-budget-check` | `scripts/loop-budget-check <slug>` | Parses RUNS/STATE deterministically and fails closed on campaign budget exhaustion or oscillation instead of asking the model to count prose. | `0` proceed · `3` malformed/missing state · `10-12` budget stops · `20-22` oscillation stops |
...
| Script | Usage signature | What it does | Exit codes |
|---|---|---|---|
| `adversary-review` | `scripts/adversary-review <slug> [task-id] [review\|review-strict] [base-ref]` | Default Claude → Codex adversarial review. Assembles a narrow, sanitized context package using the optional positional diff base and sends it to the reviewer CLI. `BASE_REF` is rejected to prevent ambient overrides. Captures the transcript to a gitignored artifact under `docs/features/<slug>/adversary/`. | `0` reviewer ran (agent reads the artifact to grade) · `2` reviewer CLI unavailable — task stays Review with `NEEDS_CROSS_MODEL_REVIEWER` · `3` usage / unsafe base · `4` sanitizer tripwire |
...
| `security-review` | `scripts/security-review <slug> [task-id] [review\|review-strict] [base-ref]` | Cross-model security review. Like `adversary-review` but pulls security-focused context, accepts an optional positional diff base, and pins `SDLC_CODEX_SECURITY_REQUIRED_MODEL` (default `gpt-5.5`). Artifact under `docs/features/<slug>/security/`. | `0` reviewer ran · `2` reviewer CLI unavailable (fall back to direct review per routing rules) · `3` usage / feature missing / model mismatch · `4` sanitizer tripwire |
...
| `pretriage-health` | `scripts/pretriage-health [--source name=path]... [--report path] [--backlog-dir path] [--create-item]` | Mines only owner-provided local snapshots, sanitizes input and output, aggregates bounded health counts, and optionally creates a deduplicated proposal with `auto: false`. It never fetches remote logs or claims work. | `0` report/item written · `3` usage / invalid source · `4` sanitizer tripwire |
| `auto-item-check` | `scripts/auto-item-check <consumer> <item-file>` | Fail-closed per-item authorization check. Pickup is permitted only when the item's `auto: true` and `auto_consumer` exactly match the requested consumer. | `0` authorized · `5` absent/revoked/mismatched authorization · `3` usage |
| `campaign-ledger` | `scripts/campaign-ledger <slug> --campaign ... --iteration ... --mode ... --routing ... --task ... --verification ... --stop-reason ... --stop-code ...` | Appends a parser-compatible campaign RUN entry and derives file count plus diff hash directly from Git, so unattended loops cannot self-report away oscillation guards. | `0` appended · `2` usage / invalid ledger or Git state |
