...
by reading the `.tier` marker). `scripts/feature-reconcile` is all-tier aware
and reads this file as both the task and evidence ledger.
...
> ledger with control inventory, baseline, candidate proof,
> data-path proof, zero untested rows, and `Result: PASS`. See the large-tier
> template's EVIDENCE.md for exact shapes.
> Every non-doc task also classifies `Application verification:`. Use
> `required` for UI/server/runtime-config work; otherwise give a specific
...
For current code-bearing tasks, keep the newest receipt and every required
typed proof in one H2 section. Reconcile intentionally refuses to borrow QA,
self-audit, or application proof from an older review attempt.
...
- Index curl: <HTTP status + public marker result>
- Port check: <free before, listener during, free after>
- Environment/config cross-check: <key names checked; never values>
- Result: PASS
```

Omit the application block only when the task declares a specific
`not-applicable —` reason. When required, run `scripts/verify-app-change` from
the full profile; `--expect` is mandatory and redirects must stay on the
declared local origin. If any in-scope row is untested, record the real count
and open a finding or follow-up task instead of marking `Result: PASS`.
...
Every current code-bearing task needs a tracked scoped clear receipt before
Review/Done. Claude-authored work uses `scripts/adversary-review`; Codex-
authored work uses `scripts/claude-adversary-review`. Same-tool review, local
transcript prose, and mutable claim dates do not satisfy the gate. Tasks
already Done at the committed adoption boundary remain legacy.
